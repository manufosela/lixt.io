...
function getPageContent(file, distPath = '') {
  console.log(`Procesando ${distdir}/${distPath}/${file}...`);
...
function processFiles(files, distPath = distdir, dir = '') {
  return new Promise(async resolve => {
    ...
    Promise.all(filePromises).then(() => {
      console.log(`Procesados ${dir}/${fileList.join(', ')}`);
      resolve();
    });
  });
}
...
async function getHtmlFiles(files, distPath = distdir, dir = '') {
  const HtmlFiles = [];
  for (const file of files) {
...
function processPath(dir = workdir, complementPath = '') {
  ...
  return new Promise(async resolve => {
    ...
    const htmlFiles = await getHtmlFiles(files, distPath, complementPath)
    console.log(htmlFiles);
    await processFiles(htmlFiles, distPath, complementPath);
    resolve();
  });
}
